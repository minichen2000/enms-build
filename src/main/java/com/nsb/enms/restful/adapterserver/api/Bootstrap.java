...
public class Bootstrap extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		Info info = new Info().title("ENMS Adapter Server").description("ENMS Adapter RESTful interface. ")
		...
		String q3WSServerUri = ConfLoader.getInstance().getConf("Q3_WS_SERVER_URI", "");
		NotificationClient client = new NotificationClient(q3WSServerUri);
		client.start();

		/*
		 * int adapterWSServerPort = ConfLoader.getInstance() .getInt(
		 * "ADP_WS_SERVER_PORT", 7778 ); NotificationServer server = new
		 * NotificationServer( adapterWSServerPort ); server.start();
		 */
