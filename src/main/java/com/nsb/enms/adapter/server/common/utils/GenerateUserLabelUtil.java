...
public class GenerateUserLabelUtil {
	public static String generateTpUserLabel(TpEntity entity) {
		...
		String userLabel = "";
		if ("labelledOpticalSPITTPBidirectional".equals(moc) || "pPITTPBidirectionalR1".equals(moc)) {
		...
		if ("tu12CTPBidirectionalR1".equals(moc)) {
			...
			userLabel = "/TU12=" + ids[3].split("=")[1] + "/" + ids[4].split("=")[1] + "/" + ids[5].split("=")[1];
			return userLabel;
		}
	...
	private static String generateTpUserLabel(String tpId, TpEntity entity, String type) {
		...
		for (String value : supporedtByObjectList) {
			...
			if (ids.length == 4) {
				...
				if (rack1.equals(rack) && shelf1.equals(shelf) && slot1.equals(slot)) {
