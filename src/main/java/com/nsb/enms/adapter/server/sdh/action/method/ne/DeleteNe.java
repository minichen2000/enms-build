...
public class DeleteNe {
	...
	private static void stopSupervision(String groupId, String neId) throws AdapterException {
		log.debug("------------Start stopSupervision-------------------");
		try {
			Process process = ExecExternalScript.run(ExternalScriptType.TSTMGR, stopSupervisionScenario, groupId, neId);
			...
			if (process.waitFor() != 0) {
				throw new AdapterException(ErrorCode.FAIL_UNSUPERVISE_NE_BY_EMLIM);
			}
		} catch (Exception e) {
	...
	private static void removeNe(String groupId, String neId) throws AdapterException {
		log.debug("------------Start removeNe-------------------");
		try {
			Process process = ExecExternalScript.run(ExternalScriptType.TSTMGR, deleteNeScenario, groupId, neId);
			InputStream inputStream = process.getInputStream();
			BufferedReader br = new BufferedReader(new InputStreamReader(inputStream));
			boolean flag = false;
			String line;
