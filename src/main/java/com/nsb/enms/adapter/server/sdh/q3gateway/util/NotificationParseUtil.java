package com.nsb.enms.adapter.server.sdh.q3gateway.util;
...
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.AttributeInfo;
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.DefinitionEntity;
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.EventType;
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.Moc;
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.Moi;
import com.nsb.enms.adapter.server.sdh.q3gateway.entity.NotificationEntity;

public class NotificationParseUtil {
	private static final Logger log = LogManager.getLogger(NotificationParseUtil.class);
	...
	public static NotificationEntity parseModificationNotification(String event) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (line.startsWith("eventTime")) {
				if (!temp.isEmpty()) {
					entity.setMoi(new Moi(temp.substring(0, temp.length() - 1)));
			...
			if (i == (length - 1)) {
				if (temp.startsWith("nAddresses:")) {
					temp = temp.replace("nAddresses:", StringUtils.EMPTY) + line;
				} else {
	...
	private static List<String> splitMsg(String event) {
		...
		event = event.replaceAll("-- SEQUENCE\\s*\\w* --", StringUtils.EMPTY).replace("-- SET OF --", StringUtils.EMPTY)
				.replace(",", StringUtils.EMPTY).replace("{", StringUtils.EMPTY).replace("}", StringUtils.EMPTY).trim();
