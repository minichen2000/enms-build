package com.nsb.enms.adapter.server.sdh.business.eq;
...
import com.nsb.enms.adapter.server.sdh.action.method.eq.GetEquipment;
...
public class AdpEqusMgr {
	...
	public void syncEquip(String groupId, String neId, int id) throws AdapterException {
		...
		for (EquipmentEntity equ : equList) {
			log.debug(equ);
			AdpEquipment newEqu = constructEquip(equ, equList, tptCoordinatorList, map, id);
			try {
	...
	private String getKey(String moi) {
		...
		if (elements.length == 6) {
			...
			return "r" + rack + "sr" + shelf + "sl" + slot + "/daughter#" + subslot;
		}
		return "";
	}
