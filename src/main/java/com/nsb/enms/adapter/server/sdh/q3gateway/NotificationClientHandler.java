package com.nsb.enms.adapter.server.sdh.q3gateway;
...
import com.nsb.enms.adapter.server.sdh.q3gateway.util.NotificationParseUtil;

public class NotificationClientHandler extends WebSocketAdapter {
	private static final Logger log = LogManager.getLogger(NotificationClientHandler.class);
	...
	public void onWebSocketClose(int statusCode, String reason) {
		...
		log.debug("WebSocket server connection closed. statusCode:" + statusCode + ", reason:" + reason);
	...
	public void onWebSocketText(String event) {
		...
		if (event.contains("eventType { stateChange }") || event.contains("eventType { attributeValueChange }")) {
			entity = NotificationParseUtil.parseModificationNotification(event);
